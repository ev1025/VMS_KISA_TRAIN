...
사용:
  python scripts/data_prep/05_hn_mine.py candidates --per-clip 16
  powershell -File scripts/data_prep/06_hn_extract.ps1
  python scripts/data_prep/05_hn_mine.py score  --weights work/onnx_final/yolov8m.onnx --conf 0.25
  python scripts/data_prep/05_hn_mine.py inject --topk 3000
"""
import sys, csv, argparse, shutil
from pathlib import Path
from collections import defaultdict
sys.stdout.reconfigure(encoding="utf-8")
import os; sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # scripts/data_prep 상위(루트)에서 config import
import config as C
...
def cmd_candidates(a):
    ...
    for p, n in sorted(place_cnt.items(), key=lambda x: -x[1]):
        print(f"   {p:14s} {n:>7,}")
    print(f"\n출력 -> {CAND_CSV}\n        {EXTRACT_HN}")
    print("다음: powershell -File scripts/data_prep/06_hn_extract.ps1   (LLM/DATA hardneg/staging 으로 선택추출)")
...
def cmd_score(a):
    ...
    print(f"출력 -> {SCORES_CSV}\n다음: python scripts/data_prep/05_hn_mine.py inject --topk 3000")
