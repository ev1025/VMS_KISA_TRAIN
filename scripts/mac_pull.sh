...
#   ./scripts/mac_pull.sh 라벨링영상    사람 원본 825편 (257GB · 약 10시간) - 라벨 이어서 하려면
#   ./scripts/mac_pull.sh 전부          위를 전부 (약 278GB · 약 11시간). 회사에 두고 가는 용
#
...
if [ "$MODE" = "채점" ] || [ "$MODE" = "전부" ]; then
  ...
  SUB="${2:-}"
  [ "$MODE" = "전부" ] && SUB="전부"        # 전부 모드는 채점도 둘 다 받는다
  case "$SUB" in
    침입) CATS="침입(30개)" ;;
    배회) CATS="배회(30개)" ;;
...
if [ "$MODE" = "라벨링영상" ] || [ "$MODE" = "전부" ]; then
  # 라벨할 것이 kisa_연구개발_사람영상 730편(남은 것)이다. 원본 영상이 있어야 대시보드에서 연다.
  # 악천후 45편과 방화 75편은 라벨이 끝나서 받지 않는다.
  say "사람 원본 영상 825편 (257GB · 약 10시간)"
  echo "  중간에 끊겨도 같은 명령을 다시 돌리면 이어받는다"
  pull "data/원본데이터/kisa_연구개발_사람영상" "data/원본데이터"

  say "손라벨·전파 저장소 (라벨을 이어서 하려면 필요)"
  pull "data/학습데이터/손라벨" "data/학습데이터"
  pull "data/학습데이터/자동라벨" "data/학습데이터"
fi
