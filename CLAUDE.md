...
## 헷갈리면 여기부터 (2026-09-16)

| 묻는 것 | 볼 곳 |
| :-- | :-- |
| **지금 시험장에서 뭐가 도나** | `docs/DEPLOY.md` (가중치·입력·판정·자주 틀리는 곳) |
| **점수가 몇 점인가** | `results/BASELINE.json` **이 파일 하나만** |
| **이 가중치가 어디서 왔나** | `results/MODELS.json` |
| 규칙을 어떻게 찾나 | `docs/EXPERIMENTS.md` 4절 |

손대고 나면 **`python scripts/check_repro.py`** 를 돌린다. 덤프 재계산이 실측과 어긋나면 코드가 갈라진 것이다.

**오늘 세 번 같은 사고가 났다. 전부 "여러 벌 중 어느 것이 진짜인지" 를 몰라서였다.**

1. 덤프가 세 벌 — 제출 경로와 다른 방식으로 만든 것을 읽어 침입이 87.72 로 어긋났다
2. 가중치가 사라짐 — 재학습 실패가 `runs/<이름>` 을 덮어써서 3에폭 실패본을 앙상블에 넣었다(방화 84.21)
3. 추론 방식이 다름 — 덤프는 타일 낱장, 제출은 배치. 문턱 근처에서 판정이 갈렸다(방화 90.00 대 100.00)

그래서 이렇게 조치했다.

- **덤프는 항목당 한 벌만 남겼다.** 나머지는 `dumps/_archive/2026-09-16/` (`왜_내렸나.txt` 동봉)
- **접은 접근 34개를 `scripts/_archive/2026-09-16/` 로 내렸다.** 남은 파이썬 90개 -> 56개
- **`scripts/fire_weight_check.py`** — 덤프를 만든 가중치가 지금도 같은 값을 내는지 대조
- **`scripts/audit_stale.py`** 가 헛것을 안 잡게 고쳤다(제출 도구를 import 하면 중복 아님, 수동 도구 목록, 알려진 별도 구현은 이유와 함께 표시)

**덤프는 파일로 남지만 가중치는 재학습에 덮어써진다. 덤프만 보고 모델을 고르지 말 것.**
