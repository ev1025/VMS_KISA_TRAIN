...
아래 말을 하기 전에 **왼쪽 명령을 먼저 돌린다.** 안 돌렸으면 그 말을 하지 않는다.
...
| 오검·미검의 원인이 "무엇 때문이다" | `python scripts/why_fp.py` · `scripts/show_fire.py` · `scripts/zone_conf.py` 로 그려서 본다 |
| 다른 모델·문서가 말한 원인을 그대로 쓸 때 | 우리 덤프로 재계산해 확인한다 (`scripts/fire_ens_try.py` · `early_late.py`) |

검사기를 새로 만들거나 고치면 `--selfcheck` 를 같이 만든다.
